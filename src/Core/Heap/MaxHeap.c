...
static void siftDown(Heap* h, const int index) {
    ...
    while (1) {
        ...
        if (left < size && h->array[left] > h->array[largest])
            largest = left;

        if (right < size && h->array[right] > h->array[largest])
            largest = right;

        if (largest == current) break;

        const int temp = h->array[current];
        h->array[current] = h->array[largest];
...
void maxHeapInsert(Heap* h, const int key) {
...
Heap* maxHeapify(int* array, const int size) {
